...
class ParticularReport(models.AbstractModel):
    ...
    _name = 'report.sst_invoice.report_complain'
    ...
    def client_order_ref(self,obj):
        if obj:
            sale_order=self.env['sale.order'].search([('name','=',obj.origin)])

            for reg in sale_order:

                return reg.client_order_ref
...
left join product_template as pp on pp.id = pt.product_tmpl_id
    ...
    def render_html(self, data=None):
        ...
        report = report_obj._get_report_from_name('sst_invoice.report_complain')
        ...
        #_amount_to_text = self.env['account.voucher']._amount_to_text(amount, currency_id)
            ...
            return report_obj.render('sst_invoice.report_complain', docargs)
