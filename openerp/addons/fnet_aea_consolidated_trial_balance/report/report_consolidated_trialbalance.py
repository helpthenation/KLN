    ...
    def lines(self, form, ids=None, done=None):
        ...
        if test != []:
            ...
            for key,value in grouped_code.iteritems():   
                ...
                i['name'] = str(i['name']).replace("KA","",1)    
                i['name'] = str(i['name']).replace("KL","",1)    
                i['name'] = str(i['name']).replace("TN","",1)    
                i['name'] = str(i['name']).replace("HO","",1)    
                com_val.update({'code':key,'name':i['name'],'balance':sum(bal)})
                #~ print "222222222222222222222222222",com_val
                if 3 not in com_val:
...
class report_consolidated_tb_trialbalance(osv.AbstractModel):
    _name = 'report.fnet_aea_consolidated_trial_balance.report_consolidated_trialbalance'
    _inherit = 'report.abstract_report'
    _template = 'fnet_aea_consolidated_trial_balance.report_consolidated_trialbalance'
