...
def _get_currency(self, cr, uid, context=None):
    ...
    print'SSSSSSSSSSSSSSSSS',user.company_id
...
class SalaryAdvancePayment(models.Model):
    ...
    def create(self, vals):
        ...
        for each_contract in search_contract:
            ...
            if not each_contract.max_percent or not each_contract.advance_date:
                raise osv.except_osv('Error!', 'Max percentage or advance days are not provided')
            adv = vals.get('advance')
            basic=0.0
            self.env.cr.execute("""
                SELECT 
                hcl.amount as amount
                FROM hr_contract hc
                JOIN hr_contract_line hcl ON hc.id = hcl.contract_id
                JOIN mline_payroll_rule mpr ON hcl.rule_id = mpr.id
                WHERE mpr.code ='BASIC' and
                hc.id = %d"""%(each_contract.id))
            base_amount=self.env.cr.dictfetchone()  
            if base_amount:
                basic = base_amount['amount']
