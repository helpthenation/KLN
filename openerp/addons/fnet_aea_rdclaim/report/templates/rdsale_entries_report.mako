...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        ...
        <%setLang(user.lang)%>
        <center><div class="header"><center>RD Sale Entry - REPORT</center></div> </center>
  ...
  <div class="act_as_tbody">
	%for cus in line['customer_details']:
		...
		<div class="act_as_row lines">
			...
			%>
			<div class="act_as_cell"  style="width: 20px;">${prod.get('opening') or 0}</div>
			<div class="act_as_cell"  style="width: 20px;">${prod.get('sale') or 0}</div>
			<div class="act_as_cell"  style="width: 20px;">${prod.get('quantity') or 0}</div>
			<div class="act_as_cell"  style="width: 20px;">${prod.get('closing') or 0}</div>
			%endfor                             
