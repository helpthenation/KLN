...
class ParticularReport(models.AbstractModel):
    ...
    _name = 'report.sst_gst_invoice.report_garments_invoices'
    ...
    def  product_value(self,obj): 
       h=self.product_date(obj)
       print'#################################',h
       lol = lambda lst, sz: [lst[i:i+sz] for i in range(0, len(lst), sz)]
       lop=lol(h,7)
       result=[]
       for i in lop:
           result.append({'value':i,'last':'no'})
       print'RESSSSSSSSSSSSSSSS', result[-1]['last']
       result[-1]['last']='yes'
       print'RE$$$$$$$$$$$$$$', result[-1]['last']
       return result
    @api.multi
    def render_html(self, data=None):
        report_obj = self.env['report']
        report = report_obj._get_report_from_name('sst_gst_invoice.report_garments_invoices')
        leave_form=self.env['account.invoice'].browse(self.ids)
        print'leave_formleave_formleave_form',leave_form.company_id
        ...
        #_amount_to_text = self.env['account.voucher']._amount_to_text(amount, currency_id)
            if order.company_id.id:
                if order.company_id.id != 1:
                    raise osv.except_osv(_("Warning!"), _("You Cannot Print This Report...!! "))                
            docargs = {
                'doc_ids': self._ids,
                #~ 'sale':sale_order,
                'doc':leave_form,
            ...
            return report_obj.render('sst_gst_invoice.report_garments_invoices', docargs)
