...
class account_invoice_report(osv.osv):
    ...
    def _select(self):
        select_str = """
            SELECT sub.id, sub.date, sub.product_id, sub.partner_id, sub.country_id,sub.section_id as section_id,sub.district_id as district_id,
                sub.payment_term, sub.period_id, sub.uom_name, sub.currency_id, sub.journal_id,
                sub.fiscal_position, sub.user_id, sub.company_id, sub.nbr, sub.type, sub.state,
                sub.categ_id, sub.date_due, sub.account_id, sub.account_line_id, sub.partner_bank_id,
                sub.product_qty, sub.price_total / cr.rate as price_total, sub.price_average /cr.rate as price_average,
                cr.rate as currency_rate, sub.residual / cr.rate as residual, sub.commercial_partner_id as commercial_partner_id,sub.days
        """
        return select_str
    ...
    def _sub_select(self):
        select_str = """
                SELECT min(ail.id) AS id,
                    ...
                    ai.partner_bank_id,
                    EXTRACT(DAY FROM ai.date_invoice) as days,
                    SUM(CASE
    ...
    def _group_by(self):
        group_by_str = """
                GROUP BY ail.product_id, ai.date_invoice, ai.id,
                    ...
                    ai.amount_total, ai.commercial_partner_id, partner.country_id,ai.section_id,ai.district_id
        """
        return group_by_str
