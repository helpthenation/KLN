...
class tn_po_report(models.Model):
    ...
    company_id=fields.Many2one('res.company',default=lambda self: self.env['res.company'].browse(self.env['res.company']._company_default_get('tn.po.report')))
    tn_po_tax_ids=fields.One2many('tn.purchase.tax.details','tax_id',"Purchase Tax Return",readonly="True")
        ...
        if self.from_date and self.to_date:
            if self.from_date<=self.to_date: 
                ...
                self.env.cr.execute(" select distinct ai.name,po.partner_id, rp.tin_vat_no, ai.id as invoice_id, ai.date_invoice,  "\
                            " po.amount_total, po.amount_tax, pc.commadity_code  from purchase_order po"\
                ...
                for line in line_list:
                    vals={
                        ...
                        'commodity_code':line['commadity_code'],
                        'invoice_id':line['invoice_id'],
                        'invoice_date':line['date_invoice'],
                        'purchase_value':line['amount_total'],
                        'tax_rate':14.5,
                        'vat_cst_paid':line['amount_tax'],
...
class tn_purchase_tax_details(models.Model):
    
    _name="tn.purchase.tax.details"
    
    tax_id=fields.Many2one('tn.po.report')
    ...
    vat_cst_paid=fields.Float('VAT CST Paid')
    categ_id=fields.Many2one('product.category','Product Category')
