...
class purchase_return_summary(osv.osv_memory):
    _name = 'purchase.return.summary'
    ...
    _description = 'Purchase Return Summary'
    _columns = {
        'company_id': fields.many2one('res.company', 'Company'),
        'from_date': fields.date('From Date', required=True),
        'to_date': fields.date('To Date', required=True),
        'landscape': fields.boolean("Landscape Mode"),
        'partner_ids':fields.many2many('res.partner', 'partner_report_po2', 'rep_id', 'partner_id', 'Customer'),
    }
    ...
    def _print_report(self, cr, uid, ids, data, context=None):
        return self.pool['report'].get_action(cr, uid, [], 'fnet_mline_sale_report.report_po_return', data=data, context=context)
