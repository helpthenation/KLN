...
class pending_purchase_register_report(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        super(pending_purchase_register_report, self).__init__(cr, uid, name, context)
    ...
    def _get_invoice_details(self, data):
        ...
        if where_sql:
            ...
            where_sql=''
        self.cr.execute(" select  ai.id,ai.date_invoice, rp.name,  ai.number, ai.amount_total, ai.amount_untaxed,ai.state"\
                        " from account_invoice ai "\
                        " join res_partner rp on (rp.id = ai.partner_id)"\
                        " where ai.type = 'in_invoice' "\
                        " and ai.state not in ('done') "\
                        " "+ where_sql +  " "\
                        " and ai.company_id = '%s' "\
...
class wrapped_pending_purchase_register_report(osv.AbstractModel):
    _name = 'report.fnet_mline_sale_report.report_purchase_register_pending'
    _inherit = 'report.abstract_report'
    _template = 'fnet_mline_sale_report.report_purchase_register_pending'
    _wrapped_report_class = pending_purchase_register_report
