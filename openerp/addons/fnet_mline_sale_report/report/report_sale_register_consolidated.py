...
class sale_register_consolidate_report(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        super(sale_register_consolidate_report, self).__init__(cr, uid, name, context)
    ...
    def _get_product_qty_april(self,data,product,month):
        if 4 in month:
            ...
            if 'company_ids' in data['form']:
                if len(data['form']['company_ids']) < 2:
                    self.cr.execute("select sum(sm.product_qty) as qty from stock_move sm JOIN product_product pt on (pt.id=sm.product_id) Join product_template pc on (pt.product_tmpl_id=pc.id) Join product_category pcc on (pc.categ_id=pcc.id) JOIN stock_picking_type spt on (spt.id=sm.picking_type_id) where spt.code='outgoing' and sm.origin_returned_move_id is NULL and sm.create_date >= '%s' and sm.create_date <= '%s' and sm.company_id='%s' and sm.product_id='%s'group by pt.default_code,sm.product_id, pc.categ_id, pc.name , pcc.name,pc.list_price" % (str(f_date),str(t_date),str(data['form']['company_ids'][0]),product))
                else:
                    cmpn = ()
                    for idek in data['form']['company_ids']:
                        cmpn = cmpn + (idek,)
                    self.cr.execute("select sum(sm.product_qty) as qty from stock_move sm JOIN product_product pt on (pt.id=sm.product_id) Join product_template pc on (pt.product_tmpl_id=pc.id) Join product_category pcc on (pc.categ_id=pcc.id) JOIN stock_picking_type spt on (spt.id=sm.picking_type_id) where spt.code='outgoing' and sm.origin_returned_move_id is NULL and sm.create_date >= '%s' and sm.create_date <= '%s' and sm.company_id in %s and sm.product_id='%s'group by pt.default_code,sm.product_id, pc.categ_id, pc.name , pcc.name,pc.list_price" % (str(f_date),str(t_date),str(cmpn),product))
            line_list = [i for i in self.cr.dictfetchall()]
            if line_list==[]:
                lis=[]
                val={}
    ...
    def _get_product_qty_total(self,data,category,categ_id):
        ...
        if 'company_ids' in data['form']:
            if len(data['form']['company_ids']) < 2:
                self.cr.execute("select sum(sm.product_qty) as qty, sum(sm.product_qty*pc.list_price) as total from stock_move sm JOIN product_product pt on (pt.id=sm.product_id) Join product_template pc on (pt.product_tmpl_id=pc.id) Join product_category pcc on (pc.categ_id=pcc.id) JOIN stock_picking_type spt on (spt.id=sm.picking_type_id) where spt.code='outgoing' and sm.origin_returned_move_id is NULL and sm.create_date >= '%s' and sm.create_date <= '%s' and sm.company_id = '%s' and pc.categ_id='%s'" % (str(from_date[0]),str(to_date[0]),str(data['form']['company_ids'][0]),categ_id))
                ...
                for record in categ_list:
                    self.cr.execute("select sum(sm.product_qty) as qty, sum(sm.product_qty*pc.list_price) as total from stock_move sm JOIN product_product pt on (pt.id=sm.product_id) Join product_template pc on (pt.product_tmpl_id=pc.id) Join product_category pcc on (pc.categ_id=pcc.id) JOIN stock_picking_type spt on (spt.id=sm.picking_type_id) where spt.code='outgoing' and sm.origin_returned_move_id is NULL and sm.create_date >= '%s' and sm.create_date <= '%s' and sm.company_id in %s and pc.categ_id='%s'" % (str(from_date[0]),str(to_date[0]),str(cmpn),record['id']))
                    total_list=[i for i in self.cr.dictfetchall()]
                    for value in total_list:
                        if value['qty'] !=None:               
                            qty=qty+value['qty']
    ...
    def _get_product_qty_april_total(self,data,category,categ_id,month):
        if 4 in month:
            ...
            if 'company_ids' in data['form']:
                if len(data['form']['company_ids']) < 2:
                    self.cr.execute("select sum(sm.product_qty) as qty, sum(sm.product_qty*pc.list_price)/1000 as total from stock_move sm JOIN product_product pt on (pt.id=sm.product_id) Join product_template pc on (pt.product_tmpl_id=pc.id) Join product_category pcc on (pc.categ_id=pcc.id) JOIN stock_picking_type spt on (spt.id=sm.picking_type_id) where spt.code='outgoing' and sm.origin_returned_move_id is NULL and sm.create_date >= '%s' and sm.create_date <= '%s' and sm.company_id='%s' and pc.categ_id='%s' " % (str(f_date),str(t_date),str(data['form']['company_ids'][0]),categ_id))
                    line_list = [i for i in self.cr.dictfetchall()]
                    if line_list==[]:
                        val['qty']=qty
                        val['total']=total
                    ...
                    categ_list=[i for i in self.cr.dictfetchall()]
                    for record in categ_list:
                        self.cr.execute("select sum(sm.product_qty) as qty, sum(sm.product_qty*pc.list_price) as total from stock_move sm JOIN product_product pt on (pt.id=sm.product_id) Join product_template pc on (pt.product_tmpl_id=pc.id) Join product_category pcc on (pc.categ_id=pcc.id) JOIN stock_picking_type spt on (spt.id=sm.picking_type_id) where spt.code='outgoing' and sm.origin_returned_move_id is NULL and sm.create_date >= '%s' and sm.create_date <= '%s' and sm.company_id in %s and pc.categ_id='%s'" % (str(f_date),str(t_date),str(cmpn),record['id']))
                        total_list=[i for i in self.cr.dictfetchall()]
                        for value in total_list:
                            if value['qty'] !=None:               
                                qty=qty+value['qty']
    ...
    def _get_total(self,data):
        ...
        if 'company_ids' in data['form']:
            if len(data['form']['company_ids']) < 2:
                self.cr.execute("select sum(sm.product_qty*pc.list_price) as total from stock_move sm JOIN product_product pt on (pt.id=sm.product_id) Join product_template pc on (pt.product_tmpl_id=pc.id) Join product_category pcc on (pc.categ_id=pcc.id) JOIN stock_picking_type spt on (spt.id=sm.picking_type_id) where spt.code='outgoing' and sm.origin_returned_move_id is NULL and sm.create_date >= '%s' and sm.create_date <= '%s' and sm.company_id='%s'" % (str(from_date[0]),str(to_date[0]),str(data['form']['company_ids'][0])))
                line_list = [i for i in self.cr.dictfetchall()]
                if line_list==[]:
                    val['total']=total
                    lis.append(val)
                ...
                for idek in data['form']['company_ids']:
                    cmpn = cmpn + (idek,)
                self.cr.execute("select sum(sm.product_qty*pc.list_price) as total from stock_move sm JOIN product_product pt on (pt.id=sm.product_id) Join product_template pc on (pt.product_tmpl_id=pc.id) Join product_category pcc on (pc.categ_id=pcc.id) JOIN stock_picking_type spt on (spt.id=sm.picking_type_id) where spt.code='outgoing' and sm.origin_returned_move_id is NULL and sm.create_date >= '%s' and sm.create_date <= '%s' and sm.company_id in %s " % (str(from_date[0]),str(to_date[0]),str(cmpn)))
                line_list = [i for i in self.cr.dictfetchall()]
                if line_list==[]:
                    val['total']=total
                    lis.append(val)
    ...
    def _get_april_total_value(self,data):
        ...
        if 4 in month:
            ...
            if 'company_ids' in data['form']:
                if len(data['form']['company_ids']) < 2:
                    self.cr.execute("select (sum(sm.product_qty*pc.list_price))/1000 as total from stock_move sm JOIN product_product pt on (pt.id=sm.product_id) Join product_template pc on (pt.product_tmpl_id=pc.id) Join product_category pcc on (pc.categ_id=pcc.id) JOIN stock_picking_type spt on (spt.id=sm.picking_type_id) where spt.code='outgoing' and sm.origin_returned_move_id is NULL and sm.create_date >= '%s' and sm.create_date <= '%s' and sm.company_id='%s'" % (str(f_date),str(t_date),str(data['form']['company_ids'][0])))
                    line_list = [i for i in self.cr.dictfetchall()]
                    if line_list==[]:
                        val['total']=total
                        lis.append(val)
                    ...
                    for idek in data['form']['company_ids']:
                        cmpn = cmpn + (idek,)
                    self.cr.execute("select (sum(sm.product_qty*pc.list_price))/1000 as total from stock_move sm JOIN product_product pt on (pt.id=sm.product_id) Join product_template pc on (pt.product_tmpl_id=pc.id) Join product_category pcc on (pc.categ_id=pcc.id) JOIN stock_picking_type spt on (spt.id=sm.picking_type_id) where spt.code='outgoing' and sm.origin_returned_move_id is NULL and sm.create_date >= '%s' and sm.create_date <= '%s' and sm.company_id in %s" % (str(f_date),str(t_date),str(cmpn)))
                    line_list = [i for i in self.cr.dictfetchall()]
                    if line_list==[]:
                        val['total']=total
                        lis.append(val)
...
class wrapped_sale_register_consolidate_report(osv.AbstractModel):
    _name = 'report.fnet_mline_sale_report.report_sale_register_consolidate'
    _inherit = 'report.abstract_report'
    _template = 'fnet_mline_sale_report.report_sale_register_consolidate'
    _wrapped_report_class = sale_register_consolidate_report
