...
class sale_product_report(report_sxw.rml_parse):
    ...
    def _get_prod_details(self, data,va):
        ...
        if where_sql:
            ...
            where_sql=''
        self.cr.execute(" SELECT pc.name as categ_name,pp.name_template as name,ail.quantity as qty,ail.price_unit as price,(ail.quantity * ail.price_unit) as tot"\
                        " FROM account_invoice ai "\
                        " JOIN account_invoice_line ail ON (ail.invoice_id = ai.id)"\
                        " JOIN product_product pp ON (pp.id = ail.product_id)"\
                        " JOIN product_template pt ON (pt.id = pp.product_tmpl_id)"\
    ...
    def _get_tot_details(self, data, va):
        ...
        if where_sql:
            ...
            where_sql=''
        self.cr.execute(" SELECT SUM(ail.quantity) as qty,SUM(ail.price_unit) as price,SUM((ail.quantity * ail.price_unit)) as tot"\
                        " FROM account_invoice ai "\
                        " JOIN account_invoice_line ail ON (ail.invoice_id = ai.id)"\
                        " JOIN product_product pp ON (pp.id = ail.product_id)"\
                        " JOIN product_template pt ON (pt.id = pp.product_tmpl_id)"\
...
class wrapped_sale_product_report(osv.AbstractModel):
    _name = 'report.fnet_mline_sale_report.report_sale_product'
    _inherit = 'report.abstract_report'
    _template = 'fnet_mline_sale_report.report_sale_product'
