    ...
    def get_lines(self, data):
        ...
        def sum_dict(dicts):
            ...
            for i in dicts:
                for k,l in i.items():
                    if k == 'name':                     
                        names_code.append(str(l))
                              
            print names_code
            fin_dict = []
            code_names = unique_list(names_code)
            for q in code_names:
                debit = 0
                credit = 0
                balance = 0
                ...
                for a in dicts:
                    if a['name'] == q:
                        ...
                        balance += a['balance']
                        level = a['level']
                fin_dict.append({'level':level,'name':q,'debit':debit,'credit':credit,'balance':balance})


            return fin_dict

        print "22222222222222", sum_dict( lines)
        #~ return sorted(sum_dict( lines), key=lambda k: k['level'])
        #~ for  s in sum_dict( lines):
            
        return sum_dict( lines)
