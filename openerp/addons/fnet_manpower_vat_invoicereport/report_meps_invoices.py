...
class ParticularReport(models.AbstractModel):
    _name = 'report.fnet_manpower_vat_invoicereport.report_meps_invoices' 
    ...
    def get_invoice_line(self,obj,val):      
        if obj:
            ...
            self.env.cr.execute(" select ail.id,ail.name as name, "\
                            " ail.uom as uom ,ail.quantity as quantity , ail.price_unit as price_unit,ail.item_no as item_no,"\
        ...
        for i in line_list:
            txt=i['name'].split('\n')
            desc.append({'prod_desc':txt,'prod_qty':i['quantity'],'prod_uom':i['uom'],'price':i['price_unit'],'subtotal':i['total'],'sno':i['item_no']})
        for i in desc:
            if len(i['prod_desc']) > 1:
                data = i['prod_desc'][0]
                i['prod_desc'][0]='<br/>'+str(data)
    ...
    def render_html(self, data=None):
        ...
        report = report_obj._get_report_from_name('fnet_manpower_vat_invoicereport.report_meps_invoices')
        ...
        for order in stock_pick.browse(self.ids):
            ...
            return report_obj.render('fnet_manpower_vat_invoicereport.report_meps_invoices', docargs)
