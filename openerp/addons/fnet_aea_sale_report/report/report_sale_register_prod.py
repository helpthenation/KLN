...
class sale_product_report(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        super(sale_product_report, self).__init__(cr, uid, name, context)
    ...
    def _get_categ_details(self, data):
        ...
        if data['form']['product_ids']:
            if len(data['form']['product_ids'])== 1:
                add = [0]
                data = data['form']['product_ids']
                product_sql.append("ail.product_id in %s" % (str(tuple(data+add))))
            else:
                product_sql.append("ail.product_id in %s" % (str(tuple(data['form']['product_ids']))))
        
        if product_sql:
            product_sql = ' and '+' and '.join(product_sql)
            str(product_sql)
        else:
            product_sql=''       
    ...
    def _get_prod_details(self, data,va):
        ...
        product_sql = []
        com = 'company_id' in data['form'] and [data['form']['company_id']] or []
        product_id = 'product_id' in data['form'] and [data['form']['product_id']] or []
        partner = 'partner_ids' in data['form'] and [data['form']['partner_ids']] or []
        from_date = 'from_date' in data['form'] and [data['form']['from_date']] or []
        to_date = 'to_date' in data['form'] and [data['form']['to_date']] or []
        if data['form']['partner_ids']:
        ...
        if product_sql:
            ...
            product_sql=''            
        self.cr.execute(" SELECT distinct ai.id,rp.name as customer, pc.name as categ_name,pp.name_template as name,ail.quantity as qty,ail.price_unit as price,(ail.quantity * ail.price_unit) as tot,((ail.quantity * ail.price_unit) * at.amount) + (ail.quantity * ail.price_unit) as vat"\
                        " FROM account_invoice ai "\
                        " JOIN account_invoice_line ail ON (ail.invoice_id = ai.id)"\
                        " JOIN product_product pp ON (pp.id = ail.product_id)"\
                        " JOIN product_template pt ON (pt.id = pp.product_tmpl_id)"\
                        ...
                        " and ai.date_invoice <= '%s' "\
                        " and pc.id = '%s' "\
                        " ORDER BY 4 "% (com[0][0],str(from_date[0]),str(to_date[0]),va))
        line_list = [i for i in self.cr.dictfetchall()]
        return line_list
...
class wrapped_sale_product_report(osv.AbstractModel):
    _name = 'report.fnet_aea_sale_report.report_sale_product'
    _inherit = 'report.abstract_report'
    _template = 'fnet_aea_sale_report.report_sale_product'
    _wrapped_report_class = sale_product_report
