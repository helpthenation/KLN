...
class purchase_register_report(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        super(purchase_register_report, self).__init__(cr, uid, name, context)
    ...
    def _get_invoice_details(self, data):
        ...
        self.cr.execute(" select  ai.id,ai.date_invoice, rp.name, rp.tin_vat_no, ai.number, ai.amount_total, ai.amount_untaxed, ai.amount_tax"\
                        ...
                        " and ai.date_invoice <= '%s' "\
                        " order by ai.number " % (com[0][0],str(from_date[0]),str(to_date[0])))
...
class wrapped_purchase_register_report(osv.AbstractModel):
    _name = 'report.fnet_aea_sale_report.report_purchase_register'
    _inherit = 'report.abstract_report'
    _template = 'fnet_aea_sale_report.report_purchase_register'
    _wrapped_report_class = purchase_register_report
