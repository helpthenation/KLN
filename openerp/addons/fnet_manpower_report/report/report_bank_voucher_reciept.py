...
class ParticularReport(osv.AbstractModel):
    _name = 'report.fnet_manpower_report.report_bank_voucher_reciept'
    ...
    def render_html(self, data=None):
        ...
        report = report_obj._get_report_from_name('fnet_manpower_report.report_bank_voucher_reciept')
        bank_obj=self.env['account.voucher'].search([('id','=',self.ids)])
        for order in bank_obj.browse(self.ids):
            if order.state:
                if order.state != 'posted':
                    raise osv.except_osv(_("Warning!"), _("You Cannot Print Report Which Is Not Posted State ...!! "))        
        docargs = {
            'doc_ids': bank_obj,
            'doc_model': report.model,
            'docs': self,
        }
        return report_obj.render('fnet_manpower_report.report_bank_voucher_reciept', docargs)                                                                                                                                                        
