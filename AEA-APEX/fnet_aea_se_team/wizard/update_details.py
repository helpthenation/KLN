...
class update_team(osv.osv_memory):
    """
    This wizard will submit the all the selected Sale Target
    """

    _name = "update.team"
    _description = "Submit the selected Sale Entry"

    def invoice_confirm(self, cr, uid, ids, context=None):
        if context is None:
            context = {}
        active_ids = context.get('active_ids', []) or []
        proxy = self.pool['sale.entry']
        for record in proxy.browse(cr, uid, active_ids, context=context):
            cr.execute("select user_id from res_partner where id = %d"%(record.partner_id))
            data=cr.dictfetchone()
            if data['user_id'] != None:
                cr.execute("update sale_entry set sr_id=%d  where id = %d"%(data['user_id'],record.ids[0]))
