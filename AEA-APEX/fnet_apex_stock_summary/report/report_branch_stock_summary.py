...
class stock_summary_apex_report(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        super(stock_summary_apex_report, self).__init__(cr, uid, name, context)
    ...
    def _get_product(self,data,val):
        ...
        for rec in list(set(default_code)):
            ...
            li.append(quan)    
        get = [item for item in li if item[1] == val and item[2] in [3,4,5,6]]
        return get
...
class wrapped_stock_ledger_apex_summary(osv.AbstractModel):
    _name = 'report.fnet_apex_stock_summary.report_stock_ledger2'
    _inherit = 'report.abstract_report'
    _template = 'fnet_apex_stock_summary.report_stock_ledger2'
    _wrapped_report_class = stock_summary_apex_report
